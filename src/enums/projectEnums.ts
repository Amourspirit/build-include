...
/** Extend enum triState to have a parse method */
...
export namespace triState {
	 ...
	 * @param minValue The minimum value that is accepted as a valid enum value.
	 * @param maxValue The maximum value that is accepted as a valid enum value.
	 * @returns The value converted into triState
	 */
	export const parse = (value: string | number, anyCase: boolean = true, minValue: number = -1, maxValue: number = 1): triState => {
		const num: number = parseInt(value.toString(), 10);
		let v: string | number;
		if (isNaN(num) === true) {
			v = value;
		...
		if (typeof v === 'number') {
			...
			if (anyCase === true) {
				v = v.toLowerCase();
				for (const k in triState) {
					if (triState.hasOwnProperty(k)) {
						const val = triState[k]
						if (typeof val === 'number') {
							const kLc = k.toLowerCase();
							if (kLc === v) {
								const e: triState = val;
								return e;
							}
						}
					}
	 ...
	 * @summary Allows of parsing of string, numbers or single char as input.  
	 * Any `value` valid for [[parse]] is also valid with this method.
	 * 
	 ...
	 * - `0`= `triState.false`
	 */
	export const parseLoose = (value: string | number): triState => {
		let result = parse(value);
		if (result > triState.unset) {
			return result;
		}
		if (typeof value === 'number') {
			result = triState.unset;
			return result;
		}
		const strValue = value.trim().toLowerCase();
		if (strValue.startsWith('t')
			|| strValue.startsWith('y')
			|| strValue === '1') {
