import { MatchBuildIncludeHtml } from "../MatchBuildIncludeHtml";
import { MatchOptionsProcess } from "../../process/MatchOptionsProcess";
const inst = new MatchBuildIncludeHtml();
...
describe('Test Properties', () => {
  ...
  test('suffix', (done) => {
    expect.assertions(1);
    expect(inst.suffix.length).toBeGreaterThan(0);
    done();
  });
  ...
  test('kind', (done) => {
    expect.assertions(1);
    expect(inst.kind).toBe('buildIncludeHtml');
    done();
  });
...
describe('MatchOptionsProcess.regexSimple', () => {
  ...
  test('MatchOptionsProcess.regexSimple match <--build_include', (done) => {
    expect.assertions(1);
    const strTest = '<--build_include';
    expect(proc.regexSimple.test(strTest)).toBeFalsy();
    done();
  });
...
describe('MatchOptionsProcess.regexMain', () => {
  ...
  test('MatchOptionsProcess.regexMain not match <!--build_include("Readme.md")', (done) => {
    expect.assertions(1);
    expect(proc.regexMain.test('<!--build_include("Readme.md")')).toBeFalsy();
    done();
  });

  test("MatchOptionsProcess.regexMain no match <!--build_include('Readme.md')\\n-->", (done) => {
    expect.assertions(1);
    const strFile = 'Readme.md';
    const strTest = `<!--build_include('${strFile}')\n-->`;
    expect(proc.regexMain.test(strTest)).toBeFalsy();
    done();
  });
